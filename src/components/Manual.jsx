    ...
    <div>
      ...
      <div className={`card manual-section ${highlighted === "generate" ? "manual-highlight-active" : ""}`} id="generate">
        ...
        <ol>
          ...
          <li>결과로 네이버 블로그(제목 3안 + 본문), LinkedIn 포스트, 쇼츠 스크립트(자막/화면 지시 2열),
            카드뉴스(7장 헤드카피/서브카피)가 각각 카드로 표시됩니다.</li>
        ...
        <ul>
          ...
          <li><strong>쇼츠 영상 생성</strong> — 쇼츠 스크립트 카드의 <strong>"🎬 쇼츠 영상 생성"</strong>을
            ...
            플랫폼에서 MP4를 요구하면 무료 온라인 변환기로 한 번 변환해서 쓰세요. <strong>배경음악(BGM)</strong>도
            선택적으로 넣을 수 있습니다 — 가지고 계신 mp3/wav 파일을 "배경음악" 칸에서 선택하면 영상 길이에
            맞춰 자동으로 반복 재생되며 함께 녹화됩니다(볼륨 조절 가능). 반드시 직접 소유했거나 로열티 프리인
      ...
      <div className={`card manual-section ${highlighted === "clipping" ? "manual-highlight-active" : ""}`} id="clipping">
        <h3>7. 클리핑 모니터링</h3>
        <ol>
          <li>키워드(쉼표 구분, 기본값: 그린플로, 오후두시랩, 탄소회계 AI)를 확인하거나 수정합니다.</li>
          <li>"지금 수집 실행"을 누르면 네이버 뉴스·블로그에서 해당 키워드 언급을 수집해 로그에 쌓습니다.</li>
          <li>ERROR 행이 연속 2건 이상 보이면 화면 상단에 경고가 뜹니다 — 네이버 API 키 만료·한도 초과를
            의심하고 설정 화면에서 키를 다시 확인하세요.</li>
        </ol>
        <h4>매일 자동으로 실행하려면</h4>
        <p>
          Windows 작업 스케줄러(또는 Mac/Linux의 cron)에 매일 09:00 <code className="kbd">POST /api/clipping/run</code>
          을 호출하도록 등록하면 완전 자동화됩니다. 네이버 검색에 잡히지 않는 언급은 Google Alerts로 보완하는
          것을 권장합니다(매뉴얼 원안과 동일).
        </p>
      </div>
      ...
      <div className={`card manual-section ${highlighted === "settings" ? "manual-highlight-active" : ""}`} id="settings">
        ...
        <ul>
          ...
          <li><strong>상주 지침</strong> — 브랜드 정의, 고정 수식어, 용어집, 톤 가이드, 금칙 표현, 블로그
            키워드 풀, SNS 해시태그 풀. 이 내용은 매 생성 요청마다 시스템 프롬프트로 자동 포함되어, 매번
            새로 지시하지 않아도 일관된 톤과 표기를 유지해줍니다. 공급자를 바꿔도 이 지침은 동일하게 적용됩니다.</li>
        </ul>
        <p className="hint">
